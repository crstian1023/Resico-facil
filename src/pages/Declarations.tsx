...
import {
  FileText, CheckCircle2, Loader2, Sparkles, Download, ArrowLeft,
  Calculator, Wallet, TrendingDown, Receipt, Pencil, Save, CreditCard, ScrollText,
} from 'lucide-react';
...
/** Robust download: anchor + download attr, fallback to window.open. Works on iOS/Android. */
...
const Declarations = () => {
  ...
  // Cálculo en vivo del ISR mientras el usuario edita
  ...
  /** Guardar cambios = recalcula (si hay edición) + guarda draft + regenera PDF + vuelve al inicio */
  const handleSaveChanges = async () => {
    if (!currentCalc) return;
    const i = Number(editIncome), e = Number(editExpenses);
  ...
  const handleDownloadExisting = async (draft: any, fileName: string) => {
    try {
      ...
      if (draft.pdf_storage_path) {
        try { url = await refreshSigned.mutateAsync(draft.pdf_storage_path); } catch {}
      }
  ...
  const handleQuickPdf = async (calcId: string, fileName: string) => {
    try {
      ...
      downloadPdf(res.pdf_url, fileName);
    } catch (e: any) {
  ...
  const handleDownloadCfdi = async (draft: any) => {
    try {
      ...
      if (draft.cfdi_demo_path) {
        try { url = await refreshCfdi.mutateAsync(draft.cfdi_demo_path); } catch {}
      }
      if (!url) {
                  ...
                  <CardContent className="p-4 space-y-3">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {hasPdf ? (
                            ...
                            <Badge variant="secondary" className="text-[10px] h-5">Borrador</Badge>
                          )}
                          <PaymentBadge status={draft?.payment_status} />
                        </div>
                      </div>
                      <div className="text-right shrink-0">
                        <p className="text-[10px] text-muted-foreground uppercase tracking-wide">ISR</p>
                    ...
                    <div className="flex flex-wrap gap-2">
                      <Button
                        ...
                        onClick={() =>
                          hasPdf
                            ? handleDownloadExisting(draft, fileName)
                            : handleQuickPdf(c.id, fileName)
                        }
  ...
  const renderFlow = () => (
    <div className="space-y-5 pb-36 md:pb-6">
      <div className="flex items-center gap-2">
        <Button variant="ghost" size="sm" onClick={() => { setMode('home'); setCurrentCalc(null); }}>
        ...
        <CardContent className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Select
                value={String(month)}
                onValueChange={(v) => { setMonth(Number(v)); setCurrentCalc(null); }}
            ...
            <div className="space-y-1.5">
              ...
              <Select
                value={String(year)}
                onValueChange={(v) => { setYear(Number(v)); setCurrentCalc(null); }}
                disabled={!!currentCalc}
              >
                <SelectTrigger className="h-12"><SelectValue /></SelectTrigger>
                <SelectContent>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="rounded-xl border bg-card p-4 space-y-2">
                ...
                <Input
                  ...
                  value={editIncome}
                  onChange={(e) => { setEditIncome(e.target.value); setDirty(true); }}
                  className="h-12 text-lg font-bold border-0 px-0 focus-visible:ring-0 shadow-none"
                />
              </div>
              <div className="rounded-xl border bg-card p-4 space-y-2">
                <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <Input
                  type="number" inputMode="decimal" min="0" step="0.01"
                  value={editExpenses}
                  onChange={(e) => { setEditExpenses(e.target.value); setDirty(true); }}
                  className="h-12 text-lg font-bold border-0 px-0 focus-visible:ring-0 shadow-none"
                />
              </div>
    ...
    <AppLayout>
      ...
      <Dialog open={!!payingDraftId} onOpenChange={(o) => !o && setPayingDraftId(null)}>
        <DialogContent className="max-w-3xl max-h-[92vh] overflow-y-auto p-0">
          ...
          {payingDraftId && (
            <DeclarationCheckout declarationId={payingDraftId} />
          )}
        </DialogContent>
      </Dialog>
    </AppLayout>
