...
const IncomeExpenses = () => {
  ...
  const loadTransactions = async () => {
    ...
    const income: Transaction[] = (incomeRes.data ?? []).map(r => ({
      id: r.id, type: 'income', amount: Number(r.amount),
      category: r.category_name ?? '', description: r.description ?? '', date: r.date,
    }));
    const expense: Transaction[] = (expenseRes.data ?? []).map(r => ({
      id: r.id, type: 'expense', amount: Number(r.amount),
      category: r.category_name ?? '', description: r.description ?? '', date: r.date,
    }));

    setTransactions(
      [...income, ...expense].sort((a, b) => b.date.localeCompare(a.date))
    );
  ...
  const handleAdd = async () => {
    ...
    const { error } = await supabase
      .from(newType === 'income' ? 'income_records' : 'expense_records')
      .insert(payload);
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <Dialog open={dialogOpen} onOpenChange={open => {
            setDialogOpen(open);
            if (!open) { setNewAmount(''); setNewCategory(''); setNewDescription(''); }
          }}>
            ...
            <DialogContent className="max-h-[90vh] overflow-y-auto">
              ...
              <div className="relative rounded-xl overflow-hidden bg-white p-4">
                ...
                {/* Colored tabs */}
                <Tabs value={newType} onValueChange={v => { setNewType(v as 'income' | 'expense'); setNewCategory(''); }}>
                ...
                <div className="space-y-2">
                  <Label>Monto</Label>
                  <Input type="number" placeholder="0.00" value={newAmount}
                    onChange={e => setNewAmount(e.target.value)} className="h-12 text-lg" />
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input placeholder="Ej: Venta de mercancía" value={newDescription}
                    onChange={e => setNewDescription(e.target.value)} className="h-12" />
                </div>
                <div className="space-y-2">
                  <Label>Fecha</Label>
                  <Input type="date" value={newDate}
                    onChange={e => setNewDate(e.target.value)} className="h-12" />
                </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <Card>
            <CardContent className="p-4 flex items-center gap-3">
              <div className="p-2 rounded-lg bg-green-100 text-green-700"><TrendingUp size={18} /></div>
              <div>
          ...
          <Card>
            <CardContent className="p-4 flex items-center gap-3">
              <div className="p-2 rounded-lg bg-red-100 text-red-700"><TrendingDown size={18} /></div>
              <div>
