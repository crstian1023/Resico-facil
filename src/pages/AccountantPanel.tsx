...
import {
  UserPlus, Search, Users, FileText, AlertCircle,
  CheckCircle2, Clock, ChevronRight, Send, Pencil, ShieldCheck,
} from 'lucide-react';
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-3">
          <Badge variant="outline" className="font-mono text-xs">{rfc}</Badge>
          ...
          <CardContent>
            {summary?.lastCalc ? (
              <div className="grid grid-cols-2 gap-3 text-sm">
                {[
                  { label: 'Periodo', value: `${summary.lastCalc.period_month}/${summary.lastCalc.period_year}`, highlight: false },
                  { label: 'ISR estimado', value: `$${Number(summary.lastCalc.estimated_tax).toLocaleString('es-MX', { minimumFractionDigits: 2 })}`, highlight: true },
                  { label: 'Ingresos', value: `$${Number(summary.lastCalc.total_income).toLocaleString('es-MX')}`, highlight: false },
                  { label: 'Gastos', value: `$${Number(summary.lastCalc.total_expenses).toLocaleString('es-MX')}`, highlight: false },
        ...
        <CardContent className="space-y-2">
          {summary?.drafts.length ? (
            summary.drafts.map((d: any) => (
              <div key={d.id} className="flex items-center justify-between text-sm p-2.5 rounded-lg border border-border bg-muted/30">
                <div className="flex items-center gap-2">
                  <FileText size={14} className="text-muted-foreground" />
                  <span className="font-medium">{d.period_month}/{d.period_year}</span>
                </div>
          ...
          <>
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-0.5">
                Gestiona tus clientes contribuyentes RESICO
              </p>
            </div>
              ...
              <TabsContent value="invite" className="mt-4">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      El cliente recibirá una solicitud y debe aceptarla desde su cuenta para que
                      puedas ver su información fiscal.
                    </CardDescription>
                  </CardHeader>
