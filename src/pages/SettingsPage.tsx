...
const fiscalSchema = z.object({
  rfc: z.string().trim().regex(/^[A-ZÑ&]{3,4}\d{6}[A-Z\d]{3}$/i, { message: 'RFC inválido' }).or(z.literal('')),
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input value={profile?.full_name || user?.user_metadata?.full_name || ''} disabled className="h-12 bg-muted" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="phone">Teléfono</Label>
              {isLoading ? <Skeleton className="h-12 w-full" /> : (
                <Input id="phone" placeholder="10 dígitos" value={phone} onChange={(e) => setPhone(e.target.value)} className="h-12" maxLength={15} />
              )}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              {isLoading ? <Skeleton className="h-12 w-full" /> : (
                <Input id="rfc" placeholder="XAXX010101000" value={rfc} onChange={(e) => setRfc(e.target.value.toUpperCase())} className="h-12 uppercase" maxLength={13} />
              )}
            </div>
            <div className="space-y-2">
              <Label htmlFor="curp">CURP</Label>
              {isLoading ? <Skeleton className="h-12 w-full" /> : (
                <Input id="curp" placeholder="18 caracteres" value={curp} onChange={(e) => setCurp(e.target.value.toUpperCase())} className="h-12 uppercase" maxLength={18} />
              )}
            </div>
            <div className="space-y-2">
              <Label htmlFor="address">Domicilio fiscal</Label>
              {isLoading ? <Skeleton className="h-12 w-full" /> : (
                <Input id="address" placeholder="Calle, número, colonia, CP" value={address} onChange={(e) => setAddress(e.target.value)} className="h-12" maxLength={500} />
              )}
            </div>
            <div className="space-y-2">
              <Label htmlFor="activity">Actividad económica principal</Label>
              {isLoading ? <Skeleton className="h-12 w-full" /> : (
                <Input id="activity" placeholder="Ej: Venta de alimentos" value={activity} onChange={(e) => setActivity(e.target.value)} className="h-12" maxLength={200} />
              )}
            </div>
            <Button size="lg" className="w-full" onClick={handleSave} disabled={isSaving || isLoading}>
              {isSaving ? <><Loader2 size={16} className="mr-2 animate-spin" /> Guardando...</> : 'Guardar cambios'}
            </Button>
          </CardContent>
        </Card>
      </div>
