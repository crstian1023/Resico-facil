-- Función que permite al contador invitar a un contribuyente por RFC.
-- Usa SECURITY DEFINER para poder leer taxpayer_profiles sin importar las RLS,
-- pero valida internamente que el invitador tenga rol 'accountant' o 'admin'.
...
DECLARE
  ...
  v_link_id       UUID;
BEGIN
  -- 1. Verificar que el invocador sea contador o admin
  SELECT role INTO v_role
  FROM public.user_roles
  WHERE user_id = v_accountant_id
  ...
  -- 2. Buscar contribuyente por RFC (lectura privilegiada)
  ...
  -- Si ya estaba activa/pendiente, ON CONFLICT no actualiza — obtenemos el id igualmente
  IF v_link_id IS NULL THEN
    SELECT id INTO v_link_id
    FROM public.accountant_client_links
    WHERE accountant_id = v_accountant_id AND client_id = v_client_id;

    -- Si ya está activo, informarlo
...
-- Revocar acceso público y permitir solo a usuarios autenticados
